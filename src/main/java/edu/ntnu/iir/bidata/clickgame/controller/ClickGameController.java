...
public class ClickGameController extends Application {
    ...
    private void setupKeyHandling(Scene gameScene, Gui gui, List<Player> playersList,
        ...
        gameScene.setOnKeyPressed(event -> {
            ...
            Integer playerId = PLAYER_KEYS.get(event.getCode());
            if (playerId != null) {
                Player player = playersList.stream()
    ...
    private void endGame(Gui gui) {
        ...
        Platform.runLater(() -> {
            ...
            // Relaunch main class (Stigespillet)
            try {
                edu.ntnu.iir.bidata.laddergame.Stigespillet.main(new String[0]);
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
    ...
    private Alert createGameOverAlert(List<Player> winners, int clicks) {
        ...
        if (winners.size() == 1) {
            ...
            tieMessage.append(" with ").append(clicks / winners.size()).append(" clicks each!");
